...
def blob_exists(blob_name: str, bucket_name: str) -> bool:
    ...
    if bucket_exists(bucket_name):
        ...
        if not isinstance(blob_name, str) or blob_name == "":
            raise TypeError("The parameter blob_name must not be empty"
...
def upload_file(
    ...
    if not isinstance(origin_file_path, str) or origin_file_path == "":
        raise ValueError("The parameter origin_file_path cannot be empty, and must be a string")
    
    if not os.path.isfile(origin_file_path):
        raise ValueError(f"The file {origin_file_path} does not exists, check the path and try again.")
    
    origin_file_path = origin_file_path.replace("\\", "/")

    # Checks for the destination_file_path parameter
    if destination_file_path == None:
        destination_file_path = origin_file_path.split("/")[-1]

    elif not isinstance(destination_file_path, str):
        raise ValueError("The parameter destination_file_path must be a string. If not passed, the file "
    ...
    logger.info(f"{origin_file_path.split('/')[-1]} stored in GCS as {destination_file_path}")
...
def delete_file(file_name: str, bucket_name: str = config.BUCKET_NAME) -> None:
    ...
    if not blob_exists(blob_name=file_name, bucket_name= bucket_name):
        raise ValueError(f"The file {file_name} does not exist in the bucket {bucket_name}")
