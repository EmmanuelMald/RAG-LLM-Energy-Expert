...
def parse_file(gcs_file_path: str, 
               chunk_size: int = 256, 
               chunk_overlap: int = 0,
    ...
    if not isinstance(gcs_file_path, str):
        raise ValueError("gcs_file_path parameter must be a string. Ex: 'folder/my_doc.pdf'")
    
    extension = gcs_file_path.split(".")[-1]

    if not extension in allowed_formats:
        raise ValueError(f"The file is in the format {extension}, which cannot be processed. Current allowed formats are: {allowed_formats.keys()}")
    
    # Step 1: Extract the data and save it into a dictionary
    file_data = allowed_formats[extension](gcs_file_path = gcs_file_path, bucket_name = bucket_name)
